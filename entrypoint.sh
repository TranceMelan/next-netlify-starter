...
echo -e "${CHECK_DONE}"
qemu_cmd="qemu-system-x86_64 -drive file=${n,,}.qcow2,if=virtio -virtfs local,path=shared,mount_tag=shared,security_model=none -m ${SERVER_MEMORY} -net nic,model=virtio"
if [ ! -e "${n,,}.qcow2" ]; then
    echo -e "${DOWNLOAD}"
